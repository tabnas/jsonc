...
# Write jsonc-grammar.jsonic into ts/src/jsonc.ts, go/jsonc.go and
...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/jsonc.go
	rm -f go/jsonc.go.bak
	git add go/jsonc.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# an edit to jsonc-grammar.jsonic compiles the previous text and succeeds.
...
# release: the crate depends on the engine and the jsonic grammar by
# path, and crates.io does not accept a path dependency, so tabnas-jsonc
# is not published. Only the constants need to stay in step.
version-rs:
