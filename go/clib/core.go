...
//
// libtabnasjsonc is the jsonc format parser as a C shared library, one of
...
import (
	...
	plug "github.com/tabnas/jsonc/go"
	host "github.com/tabnas/jsonic/go"
)

const (
	templateVersion = "v1"
	libName         = "libtabnasjsonc"
	formatName      = "jsonc"
	valueOut        = true
)
...
// newParser builds one engine with the jsonc grammar installed
...
func newParser() (parseFn, error) {
	j := host.Make(); if err := j.Use(plug.Jsonc); err != nil { return nil, err }; return j.Parse, nil
}
...
// parseWith answers whether src is in the jsonc language — and,
