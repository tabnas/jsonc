...
// Package main builds the C-ABI shared library: libtabnasjsonc.
//
// tabnas-clib-template: v1 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasjsonc.so ./clib
//
// jsonc TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a jsonc parser and returns a handle to it.
...
// tabnas_parse checks one input against the jsonc grammar.
//
//export tabnas_parse
