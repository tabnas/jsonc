...
// port of. Mirrors ts/test/version.test.ts and go/version_test.go, and
// like them it is fatal, never skipped: a version check that silently
// does not run is the failure mode it exists to prevent.
...
fn version_looks_like_a_semver() {
    let parts: Vec<&str> = tabnas_jsonc::VERSION.split('.').collect();
    assert_eq!(
        parts.len(),
        3,
        "VERSION is not x.y.z: {}",
        tabnas_jsonc::VERSION
    );
    for part in parts {
        assert!(
