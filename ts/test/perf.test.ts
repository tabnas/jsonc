...
//
// jsonc has NO convenience parse() entry point: it is a plugin users install
// themselves (new Tabnas().use(jsonic).use(Jsonc)). So there is nothing for
// the module to cache — the regression we can guard is the *usage*: build ONE
// instance and reuse it for many parses, never rebuilding the (expensive)
// engine + grammar per parse. Building the grammar dominates a parse.
//
...
describe('perf', () => {
  test('reusing one instance stays linear and beats rebuild-per-parse', () => {
    ...
    for (let i = 0; i < 100; i++) {
      assert.deepEqual(j.parse(SRC), { a: 1, b: [2, 3], c: 'x' })
    }
    ...
    t0 = process.hrtime.bigint()
    for (let i = 0; i < N; i++) {
      const rj = new Tabnas().use(jsonic).use(Jsonc)
      rj.parse(SRC)
    }
