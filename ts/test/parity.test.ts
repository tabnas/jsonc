...
// JSONC rejects unprintable characters, so NUL is escapable here as \0.
  ...
  describe('spec: ' + file, () => {
    ...
    for (const row of rows) {
      test(`row ${row.line}: ${label(row.input)}`, () => {
        ...
        const tn = new Tabnas().use(jsonic).use(Jsonc, opts)

        if (row.expected.startsWith('ERROR')) {
          const want = row.expected.slice('ERROR'.length).replace(/^:/, '')
          // A bare `ERROR` used to be satisfied by ANY throw, because the
          // validator returned true unconditionally when the substring was
          // empty — a TypeError from a broken harness passed as a conformance
          // result. Require a real Tabnas parse error, and check the substring
          // when one is given.
          let err: any
          try {
            const got = tn.parse(row.input)
            assert.fail(
              `${file}:${row.line}: expected ${row.expected}, parsed to ${JSON.stringify(got)}`,
            )
          } catch (e: any) {
          ...
          assert.strictEqual(
            ...
            'TabnasError',
            `${file}:${row.line}: expected a Tabnas parse error, got ` +
              `${err?.constructor?.name}: ${err?.message}`,
          )
          if ('' !== want) {
            assert.ok(
              String(err.message).includes(want) || String(err.code).includes(want),
